"""
MotionLibraryManager — Centralised synchroniser for ASE style data.
Fully vectorised, O(1) step performance, VRAM-optimized, and mathematically pure.
"""
import json
import torch
import numpy as np

class MotionLibraryManager:
    def __init__(self, library_path: str, latent_dim: int = 16,
                 mixup_prob: float = 0.15, mixup_alpha_range: tuple = (0.2, 0.8),
                 device: str = "cuda"):
        self.device = device
        self.latent_dim = latent_dim
        self.mixup_prob = mixup_prob
        self.mixup_alpha_range = mixup_alpha_range

        with open(library_path, "r") as f:
            self.library = json.load(f)

        self.mocap_dim = self.library["mocap_dim"]
        
        # 🚨 FIX 1: Purge invalid clips to prevent index bounds errors and fake zero-transitions
        self.clips = [clip for clip in self.library["clips"] if len(clip["frames"]) > 1]
        self.num_clips = len(self.clips)

        # ---- Batch CPU Construction (Prevents VRAM Fragmentation) ----
        z_list = [clip["latent_z"] for clip in self.clips]
        kin_sigs = [[
            clip.get("kinematic_signature", {}).get("mean_base_vel_mag", 0.0),
            clip.get("kinematic_signature", {}).get("mean_vertical_com_disp", 0.0),
            clip.get("kinematic_signature", {}).get("mean_contact_duty", 0.5)
        ] for clip in self.clips]
        
        start_poses = [clip["frames"][0]["joint_pos"] for clip in self.clips]
        start_vels = [clip["frames"][0].get("joint_vel", [0.0]*self.mocap_dim) for clip in self.clips]
        first_frame_contacts = [clip["frames"][0].get("contact_schedule", [0,0,0,0]) for clip in self.clips]
        durations = [clip["duration_s"] for clip in self.clips]
        total_frames = [len(clip["frames"]) for clip in self.clips]
        max_frames = max(total_frames)

        # Allocate contiguous CPU arrays
        pad_jp = np.zeros((self.num_clips, max_frames, self.mocap_dim), dtype=np.float32)
        pad_cs = np.zeros((self.num_clips, max_frames, 4), dtype=np.float32)
        pad_sm = np.ones((self.num_clips, max_frames), dtype=np.float32)
        pad_rhy = np.zeros((self.num_clips, max_frames), dtype=np.float32)
        clip_types = np.zeros(self.num_clips, dtype=np.int64)
        type_map = {"loco": 0, "combat": 1, "dance": 2, "precision": 3}

        for i, clip in enumerate(self.clips):
            frames_count = len(clip["frames"])
            pad_jp[i, :frames_count] = [f["joint_pos"] for f in clip["frames"]]
            pad_cs[i, :frames_count] = [f.get("contact_schedule", [0,0,0,0]) for f in clip["frames"]]
            pad_sm[i, :frames_count] = [f.get("stiffness_mult", 1.0) for f in clip["frames"]]
            pad_rhy[i, :frames_count] = [f.get("rhythm_beat", 0.0) for f in clip["frames"]]
            clip_types[i] = type_map.get(clip.get("skill_type", "loco"), 0)

        # ---- One-Shot GPU Transfers (Type-Safe) ----
        self.z_codes = torch.as_tensor(z_list, dtype=torch.float32, device=device)
        self.kin_sigs = torch.as_tensor(kin_sigs, dtype=torch.float32, device=device)
        self.start_poses = torch.as_tensor(start_poses, dtype=torch.float32, device=device)
        self.start_vels = torch.as_tensor(start_vels, dtype=torch.float32, device=device)
        self.first_frame_contacts = torch.as_tensor(first_frame_contacts, dtype=torch.float32, device=device)
        self.clip_durations = torch.as_tensor(durations, dtype=torch.float32, device=device)
        self.total_frames_per_clip = torch.as_tensor(total_frames, dtype=torch.long, device=device)

        self.padded_joint_pos = torch.as_tensor(pad_jp, device=device)
        self.padded_contact_schedules = torch.as_tensor(pad_cs, device=device)
        self.padded_stiffness = torch.as_tensor(pad_sm, device=device)
        self.padded_rhythm = torch.as_tensor(pad_rhy, device=device)
        self.clip_skill_types = torch.as_tensor(clip_types, device=device)

    def _get_safe_mixup_indices(self, primary_ids: torch.Tensor, k: int = 5) -> torch.Tensor:
        safe_k = min(k, self.num_clips - 1)
        if safe_k <= 0:
            return primary_ids
        z_primary = self.z_codes[primary_ids]
        dists = torch.cdist(z_primary, self.z_codes)
        nearest = torch.topk(dists, safe_k + 1, dim=-1, largest=False).indices[:, 1:]
        rand_sel = torch.randint(0, safe_k, (len(primary_ids), 1), device=self.device)
        return torch.gather(nearest, 1, rand_sel).squeeze(-1)

    def sample(self, env, env_ids: torch.Tensor):
        data = env.extras["ase_data"]
        num_resets = len(env_ids)

        if data["desired_contacts"] is None:
            data["desired_contacts"] = torch.zeros((env.num_envs, 4), device=self.device)
            
        if data["start_joint_pos"] is None:
            # 🚨 FIX: Initialize the buffer with the safe Athletic Stance, not zeros!
            data["start_joint_pos"] = env.scene["robot"].data.default_joint_pos.clone()
            data["start_joint_vel"] = env.scene["robot"].data.default_joint_vel.clone()
            
        if not hasattr(env, "_clip_idx"):
            env._clip_idx = torch.zeros(env.num_envs, dtype=torch.long, device=self.device)
            env._clip_total_frames = torch.zeros(env.num_envs, dtype=torch.long, device=self.device)

        is_skill = torch.rand(num_resets, device=self.device) < 0.5
        is_loco = ~is_skill

        if is_loco.any():
            loco_ids = env_ids[is_loco]
            data["z"][loco_ids] = 0.0
            data["phase"][loco_ids] = 0.0
            data["is_skill_mode"][loco_ids] = False
            data["is_loco_mode"][loco_ids] = True
            data["is_combat_mode"][loco_ids] = False
            data["is_dance_mode"][loco_ids] = False
            data["is_precision_mode"][loco_ids] = False
            data["desired_contacts"][loco_ids] = 0.0
            data["stiffness_mult"][loco_ids] = 1.0
            
            # 🚨 FIX: Re-apply the Athletic Stance for Loco resets so they don't inherit previous skill poses
            data["start_joint_pos"][loco_ids] = env.scene["robot"].data.default_joint_pos[loco_ids]
            data["start_joint_vel"][loco_ids] = env.scene["robot"].data.default_joint_vel[loco_ids]
            
        if is_skill.any():
            skill_ids = env_ids[is_skill]
            n_skills = len(skill_ids)

            primary_idx = torch.randint(0, self.num_clips, (n_skills,), device=self.device)
            z_sampled = self.z_codes[primary_idx]

            do_mixup = torch.rand(n_skills, device=self.device) < self.mixup_prob
            if do_mixup.any():
                secondary_idx = self._get_safe_mixup_indices(primary_idx)
                z_secondary = self.z_codes[secondary_idx]
                alphas = torch.empty((n_skills, 1), device=self.device).uniform_(*self.mixup_alpha_range)
                z_mixed = alphas * z_sampled + (1.0 - alphas) * z_secondary
                z_sampled = torch.where(do_mixup.unsqueeze(1), z_mixed, z_sampled)

            data["z"][skill_ids] = z_sampled
            data["phase"][skill_ids] = 0.0
            data["is_skill_mode"][skill_ids] = True
            data["is_loco_mode"][skill_ids] = False

            sampled_types = self.clip_skill_types[primary_idx]
            data["is_combat_mode"][skill_ids] = (sampled_types == 1)
            data["is_dance_mode"][skill_ids] = (sampled_types == 2)
            data["is_precision_mode"][skill_ids] = (sampled_types == 3)

            env._clip_idx[skill_ids] = primary_idx
            env._clip_total_frames[skill_ids] = self.total_frames_per_clip[primary_idx]

            data["desired_contacts"][skill_ids] = self.first_frame_contacts[primary_idx]
            data["start_joint_pos"][skill_ids] = self.start_poses[primary_idx]
            data["start_joint_vel"][skill_ids] = self.start_vels[primary_idx]
            data["rhythm_array"][skill_ids, 0] = self.padded_rhythm[primary_idx, 0]

    def advance_phase(self, env):
        data = env.extras["ase_data"]
        # 🚨 FIX 2: Use canonical Isaac Lab property for robustness
        dt = env.step_dt 
        active_mask = data["is_skill_mode"]

        if not active_mask.any():
            return

        clip_idx = env._clip_idx[active_mask]
        durations = self.clip_durations[clip_idx]
        total_frames = env._clip_total_frames[active_mask]

        phase_inc = dt / durations
        data["phase"][active_mask] += phase_inc
        data["phase"][active_mask] %= 1.0

        current_frames = (data["phase"][active_mask] * total_frames).long()
        current_frames = torch.clamp(current_frames, max=total_frames - 1)

        data["desired_contacts"][active_mask] = self.padded_contact_schedules[clip_idx, current_frames]
        data["stiffness_mult"][active_mask] = self.padded_stiffness[clip_idx, current_frames]
        data["rhythm_array"][active_mask, 0] = self.padded_rhythm[clip_idx, current_frames]

    def sample_real_transitions(self, batch_size: int, device: torch.device) -> tuple:
        """
        Vectorised sampling of real transitions (s, s_next, z).
        Returns 39-dim joint positions to match the Discriminator's kinematic focus.
        """
        clip_indices = torch.randint(0, self.num_clips, (batch_size,), device=device)
        max_frame = self.total_frames_per_clip[clip_indices] - 1  # Guaranteed >= 1 due to __init__ filter
        
        # 🚨 FIX 3: Unbiased, vectorised uniform sampling without modulo wrap-around
        frame_indices = (torch.rand(batch_size, device=device) * max_frame.float()).long()
        next_frame_indices = frame_indices + 1

        s_joints = self.padded_joint_pos[clip_indices, frame_indices]
        s_next_joints = self.padded_joint_pos[clip_indices, next_frame_indices]
        z = self.z_codes[clip_indices]

        # Returns pure 39-dim kinematic state. 
        # The training loop will slice the policy's observation (obs[:, 3:42]) to match this.
        return s_joints, s_next_joints, z